<script lang="ts">
  ...
  let decision = $state<"valide" | "accepte_sous_reserve" | "refuse" | "">("");
  let comment = $state("");
  let error = $state("");
  let loading = $state(false);
  let alreadyDecided = $state(false);
  ...
  async function handleSubmit() {
    ...
    if (
      ...
      error = "Un commentaire est obligatoire pour cette décision.";
      return;
    }
